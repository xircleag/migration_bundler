...
module MonkeyButler
  module Targets
    class MavenTarget < MonkeyButler::Targets::Base
      ...
      def validate
        fail Error, "Invalid configuration: maven.repo is not configured." if maven_url.nil?
        fail Error, "Invalid configuration: maven.username is not configured." if maven_username.nil?
        fail Error, "Invalid configuration: maven.password is not configured." if maven_password.nil?
      end
